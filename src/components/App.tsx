...
> {
  ...
  private initAudioEngine = async () => {
    ...
    audioEngine.setChannels(channelsInitialConfig)
  ...
  private loadAndSetAudioFont = async (audioFontId: AudioFontId) => {
    ...
    await audioEngine.loadAudioFont(audioFontId)
    settingsStore.audioFontId = audioFontId
    this.setState({ isLoadingAudioFont: false })
  }

  private _generateStaffTicks: (
    noteCards: NoteCardType[],
    modifiers: NoteModifiers,
    rests: number,
  ) => ReturnType<typeof generateStaffTicks> = memoize(
    (noteCards, modifiers, rests) =>
      generateStaffTicks({
        noteCards,
        modifiers,
        rests,
  ...
  private getStaffTicks = (): {
    staffTicks: StaffTick[]
    tickLabels: { [tickIndex: number]: string }
    staffTicksPerCard: { [cardId: string]: StaffTick[] }
  } => {
    if (!sessionStore.activeSession) {
      return { staffTicks: [], tickLabels: [], staffTicksPerCard: {} }
    }
  ...
  private getPianoNoteRange = () => {
    const { staffTicks } = this.getStaffTicks()
    return this._getPianoNoteRange(this.state.width, staffTicks)
  }
  ...
  private updatePlaybackNotesFromActiveSession = async () => {
    const activeSession = sessionStore.activeSession
    console.log('updatePlaybackNotesFromActiveSession', activeSession)
    if (!activeSession) {
      return
    }
  ...
  /**
   * This callback gets called by the audio engine on each tick event, inside it the animation state gets updated
   */
  private audioEngineTickCallback: TickCallback = ({
    event,
  }: {
    ...
    this.setState(state => {
      ...
      const nextStaffTickIndex = event.data.staffTickIndex
      if (nextStaffTickIndex == null) {
        return null
      }

      const { staffTicks } = this.getStaffTicks()
      const nextStaffTick = staffTicks[nextStaffTickIndex]
      if (!nextStaffTick) {
        return null
      }
  ...
  private canTransposeAllCardsUpOctave = () => {
    const { staffTicks } = this.getStaffTicks()
    const highestNote = this.getHighestNoteWithinSession(staffTicks)
    return highestNote
  ...
  private canTransposeAllCardsDownOctave = () => {
    const { staffTicks } = this.getStaffTicks()
    const lowestNote = this.getLowestNoteWithinSession(staffTicks)
    return lowestNote
  ...
  private getPianoKeyboardNoteColor = midi => {
    ...
    const { staffTicks, staffTicksPerCard } = this.getStaffTicks()
  ...
  private getNotesStaffScaleFactor = (isMobile: boolean) => {
    ...
    if (staffTicks.length > 96) {
      scale *= 0.7
    } else if (staffTicks.length > 64) {
      scale *= 0.75
    } else if (staffTicks.length > 32) {
      scale *= 0.8
    } else if (staffTicks.length > 16) {
      scale *= 0.9
    }
